...
class slip_detection_reaction:
    ...
    def defect_mask(self, img):
        ...
        im_g = ((img[:,:,1].astype(np.int16)- background +50)*2+20)
    ...
    def dispOpticalFlow(self,im_cal,x,y):
        ...
        cv2.imshow("force_flow_2",img.astype(np.uint8))
        cv2.waitKey(1)
    ...
    def call_back(self,data):
        ...
        # cv2.imwrite('sample_img_gelsight2_small.png', raw_imag.astype(np.uint8))
        ...
        if not self.con_flag:
            ...
            if self.initial_flag:
                ...
                self.img_blur = cv2.GaussianBlur(imgwc.astype(np.float32),(31,31),31)
            ...
            # im_cal = (imgwc-self.img_blur)+150
            # cv2.imshow('gray_image',im_cal.astype(np.uint8))
            # cv2.waitKey(1)
            # print im_cal.shape
