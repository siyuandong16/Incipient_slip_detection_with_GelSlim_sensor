...
class slip_detection_reaction:
    ...
    def flow_calculate_v4(self,keypoints2):
    ...
    def flow_calculate_v5(self,keypoints2):
        x2, y2, u, v, x1_paired, y1_paired, x2_paired, y2_paired  = [], [], [], [], [], [], [], []
        ...
        inbound_check = self.ROI[np.array(y2).astype(np.uint16),np.array(x2).astype(np.uint16)]*np.array(range(len(x2)))
    ...
    def contact_detection(self,im):
        ...
        # contact_general = cv2.dilate(contact, self.kernal3, iterations=1).astype(np.uint8)
        # contact[:5,:] = 0
        # contact[-5:,:] = 0
        # contact[:,:5] = 0
        # contact[:,-5:] = 0
    ...
    def dispOpticalFlow(self,im_cal,x,y):
        ...
        for i in range(self.u_sum.shape[0]):
             ...
             mask2 = cv2.line(mask2, (int(x[i]+self.u_estimate[i]*amf),int(y[i]+self.v_estimate[i]*amf)),(int(x[i]),int(y[i])), [0, 0, 80], 2)
             mask2 = cv2.line(mask2, (int(x[i]+self.u_sum[i]*amf),int(y[i]+self.v_sum[i]*amf)),(int(x[i]),int(y[i])), [0, 80, 80], 2)
    ...
    def call_back(self,data):
        ...
        if not self.con_flag:
            ...
            if self.refresh:
                keypoints = self.find_dots(final_image)
                self.x1,self.y1,x2,y2,u,v = self.flow_calculate_v5(keypoints)
            else:
                keypoints = self.find_dots(final_image*self.ROI)
                self.x1,self.y1,x2,y2,u,v = self.flow_calculate_v4(keypoints)
